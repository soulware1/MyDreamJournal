...
function SMODS.current_mod.calculate(self, context)
	...
	if not G.GAME.current_round.MDJ_construction_jokers_ranks and not G.GAME.current_round.MDJ_construction_jokers_temp_ranks and not G.GAME.current_round.MDJ_construction_jokers_displayed_ranks and not G.GAME.current_round.MDJ_construction_jokers_first_hand then
		...
		G.GAME.current_round.last_payout = G.GAME.current_round.last_payout or to_big(0)
		G.GAME.current_round.MDJ_electron_beam = G.GAME.current_round.MDJ_electron_beam or to_big(0)
		G.GAME.current_round.MDJ_pluschips = G.GAME.current_round.MDJ_pluschips or {}
		G.GAME.current_round.MDJ_pluschips_mean = G.GAME.current_round.MDJ_pluschips_mean or 0
		G.GAME.current_round.MDJ_pluschips_mode = G.GAME.current_round.MDJ_pluschips_mode or 0
		G.GAME.current_round.MDJ_pluschips_median = G.GAME.current_round.MDJ_pluschips_median or 0
		G.GAME.current_round.MDJ_plusmult = G.GAME.current_round.MDJ_plusmult or {}
		G.GAME.current_round.MDJ_plusmult_mean = G.GAME.current_round.MDJ_plusmult_mean or 0
		G.GAME.current_round.MDJ_plusmult_mode = G.GAME.current_round.MDJ_plusmult_mode or 0
		G.GAME.current_round.MDJ_plusmult_median = G.GAME.current_round.MDJ_plusmult_median or 0
		G.GAME.current_round.MDJ_xchips = G.GAME.current_round.MDJ_xchips or {}
		G.GAME.current_round.MDJ_xchips_mean = G.GAME.current_round.MDJ_xchips_mean or 0
		G.GAME.current_round.MDJ_xchips_mode = G.GAME.current_round.MDJ_xchips_mode or 0
		G.GAME.current_round.MDJ_xchips_median = G.GAME.current_round.MDJ_xchips_median or 0
		G.GAME.current_round.MDJ_xmult = G.GAME.current_round.MDJ_xmult or {}
		G.GAME.current_round.MDJ_xmult_mean = G.GAME.current_round.MDJ_xmult_mean or 0
		G.GAME.current_round.MDJ_xmult_mode = G.GAME.current_round.MDJ_xmult_mode or 0
		G.GAME.current_round.MDJ_xmult_median = G.GAME.current_round.MDJ_xmult_median or 0
	end
...
-- hooks for this mod's keys
local keyhook = SMODS.calculate_individual_effect
SMODS.calculate_individual_effect = function(effect, scored_card, key, amount, from_edition)
	if key == 'set_mult' then
		mult = mod_mult(SetDigits(mult, amount))
		if not Talisman or not Talisman.config_file.disable_anims then
			MyDreamJournal.card_eval_status_text_eq(scored_card or effect.card or effect.focus, 'mult', amount, percent, nil, nil, "Mult = "..SetDigits(mult, amount), G.C.RED)
		end
		return true
	end
	if key == 'set_chips' then
		hand_chips = mod_chips(SetDigits(hand_chips, amount))
		if not Talisman or not Talisman.config_file.disable_anims then
			MyDreamJournal.card_eval_status_text_eq(scored_card or effect.card or effect.focus, 'chips', amount, percent, nil, nil, "Chips = "..SetDigits(hand_chips, amount), G.C.BLUE)
		end
		return true
	end
	...
	if key == 'set_visible_mult' then
		...
		amount = SetVisibleDigits(mults.current, amount)
		mult = mod_mult(amount)
		if not Talisman or not Talisman.config_file.disable_anims then
			MyDreamJournal.card_eval_status_text_eq(scored_card or effect.card or effect.focus, 'mult', amount, percent, nil, nil, "Mult = "..mults.current, G.C.RED)
		end
		return true
	end
	if key == 'set_visible_chips' then
		local chips = SMODS.Scoring_Parameters["chips"]
		amount = SetVisibleDigits(chips.current, amount)
	...
	if key == 'percent_mult' then
		local mults = SMODS.Scoring_Parameters["mult"]
		amount = mults.current+mults.current*(amount/100)
		mult = mod_mult(mults.current+amount)
		if not Talisman or not Talisman.config_file.disable_anims then
	...
	if key == 'percent_chips' then
		local chips = SMODS.Scoring_Parameters["chips"]
		amount = chips.current+chips.current*(amount/100)
...
-- add in the equals if no entropy :sob:
	...
	local ret = keyhook(effect, scored_card, key, amount, from_edition)
	if ret then return ret end
end


-- large empty area so i can notice when i'm going into the other hook
