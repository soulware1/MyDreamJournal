...
SMODS.Consumable {
    ...
    use = function(self, card, area, copier)
        ...
        local enhancement_conv = rightmost.config.center.key
        assert(enhancement_conv, "ok what the fuck")
        G.E_MANAGER:add_event(Event({
            trigger = 'after',
            delay = 0.4,
            func = function()
        ...
        for i = 1, #G.hand.highlighted do
            G.E_MANAGER:add_event(Event({
                ...
                delay = 0.1,
                func = function()
                    G.hand.highlighted[i]:set_ability(enhancement_conv)
                    return true
                end
            }))
        end
    ...
    can_use = function(self, card)
        ...
        return G.hand and #G.hand.highlighted > 0 and #G.hand.highlighted <= card.ability.max_highlighted and rightmost.config.center.key ~= "m_base"
    end
}

SMODS.Consumable {
    key = 'hardware_photodiode',
    atlas = "placeholder",
    set = 'Hardware',
    pos = { x = 1, y = 0 },
    config = { max_highlighted = 3 },
    ...
    use = function(self, card, area, copier)
        ...
        local seal_conv = rightmost.seal
        assert(seal_conv, "ok what the wunk")
        G.E_MANAGER:add_event(Event({
            trigger = 'after',
            delay = 0.4,
            func = function()
    ...
    can_use = function(self, card)
        ...
        return G.hand and #G.hand.highlighted > 0 and #G.hand.highlighted <= card.ability.max_highlighted and rightmost.seal
    end
}
