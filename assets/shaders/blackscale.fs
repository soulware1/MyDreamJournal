...
extern PRECISION vec2 blackscale;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    vec4 hslInvert = tex;
    number greyscale = 1;
    if (hslInvert.x < greyscale) greyscale = hslInvert.x;
    if (hslInvert.y < greyscale) greyscale = hslInvert.y;
    if (hslInvert.z < greyscale) greyscale = hslInvert.z;
    hslInvert.x = greyscale;
    hslInvert.y = greyscale;
	hslInvert.z = greyscale;

    tex = hslInvert;
	if (blackscale.y > blackscale.y*2) {tex = vec4(0);}
    // required
